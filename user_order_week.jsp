            ...
            <div class="container-fluid pt-4 px-4" >
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">all orders this week</h6>
                        <a href="">Show All</a>
                    </div>
                        ...
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            <thead>
                                <tr class="text-white">
                                    ...
                                    <th scope="">ID_order</th>
                                    <th scope="">full_name</th>
                                    <th scope="">email</th>
                                    <th scope="">date </th>
                                      <th scope="">delivery_Date </th>
               ...
               PreparedStatement ps2 = con.prepareStatement("SELECT * FROM customer INNER JOIN orders INNER JOIN payment ON orders.customer_ID = customer.customer_ID AND payment.payment_ID = orders.payment_ID where DATE(order_date) >= DATE_SUB(CURDATE(), INTERVAL 1 WEEK) ");
               ResultSet rs2 = ps2.executeQuery();
while(rs2.next()){%>   <tr>
                                    <td><input class="form-check-input" type="checkbox"></td>
                                    <td name="ID_order"><%=rs2.getString(9) %></td>
                                    <td name="full_name"><%=rs2.getString(3)%><%=" "%> <%=rs2.getString(4)%></td>
                                     <td name="email"><%=rs2.getString(2) %></td>
                               <td name="date"><%=rs2.getString(18) %></td>
                                <td name="delivery"><%=rs2.getString(19) %></td>
